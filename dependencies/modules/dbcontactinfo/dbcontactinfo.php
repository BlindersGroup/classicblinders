...
class Dbcontactinfo extends Module
{
        ...
        $forms[] = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'hidden',
                        'label' => $this->l('Favoritos en top'),
                        'name' => 'DBCONTACTINFO_WISHLIST',
                        'is_bool' => true,
                        'desc' => $this->l('¿Mostrar enlace a los favoritos?'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
    ...
    protected function getConfigFormValues()
    {
        return array(
            ...
            'DBCONTACTINFO_WISHLIST' => Configuration::get('DBCONTACTINFO_WISHLIST'),
        );
    }
    ...
    public function hookDisplayTop()
    {
        if(!Context::getContext()->isMobile()) {
            ...
            if(Module::isEnabled('blockwishlist')){
                $display_wishlist = Configuration::get('DBCONTACTINFO_WISHLIST');
            } else {
                $display_wishlist = 0;
            }
            ...
            $this->context->smarty->assign('wishlist', $display_wishlist);
    ...
    public function hookDisplayTopCenter()
    {
        if(!Context::getContext()->isMobile()) {
            ...
            $this->context->smarty->assign('phone', $phone);
            $this->context->smarty->assign('phone_top', Configuration::get('DBCONTACTINFO_DISPLAYTOP_PHONE'));
            return $this->fetch('module:dbcontactinfo/views/templates/hook/displaytopcenter.tpl');
        }
    }
    ...
    public function hookdisplayOrderConfirmation2($params)
    {
        ...
        $this->context->smarty->assign('email', $email);
        $this->context->smarty->assign('horario', $horario);
        return $this->fetch('module:dbcontactinfo/views/templates/hook/order-confirmation.tpl');
    }
}
