...
class Dbbestseller extends Module
{
    protected $config_form = false;

    public function __construct()
    {
        $this->name = 'dbbestseller';
        $this->tab = 'front_office_features';
        $this->version = '1.1.0';
        $this->author = 'dbblinders';
        ...
        $this->displayName = $this->l('DB Best Seller');
        $this->description = $this->l('Listado de productos de una categoría específica en el footer de la ficha de producto');
    ...
    {
        Configuration::updateValue('DBBESTSELLER_CATEGORY', 12);
    ...
    public function getContent()
    {
         ...
         */
        if (((bool)Tools::isSubmit('submitDbbestsellerModule')) == true) {
            $this->postProcess();
        }
    ...
    protected function renderForm()
    {
        ...
        $helper->submit_action = 'submitDbbestsellerModule';
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        'col' => 3,
                        'type' => 'text',
                        'desc' => $this->l('Number of best-selling products in the categories'),
                        'name' => 'DBBESTSELLER_CATEGORY',
    ...
    {
        return array(
            'DBBESTSELLER_CATEGORY' => Configuration::get('DBBESTSELLER_CATEGORY'),
        );
    }
    ...
    public function hookDisplayFooterCategory()
    {
        ...
        if($controller == 'category') {
            $templateFile = 'module:dbbestseller/views/templates/hook/category.tpl';
            $id_category = Tools::getValue('id_category');
            if (!$this->isCached($templateFile, $this->getCacheId('dbbestseller_'.$id_category))) {
                $category = new Category($id_category, $id_lang);
                $limit = (int)Configuration::get('DBBESTSELLER_CATEGORY');
                $products = $this->getBestSeller($id_lang, $limit, $id_category);
                if (is_array($products) && count($products) > 0) {
                    ...
                    return $this->fetch($templateFile, $this->getCacheId('dbbestseller_'.$id_category));
                }
            } else {
                return $this->fetch($templateFile, $this->getCacheId('dbbestseller_'.$id_category));
            }
            ...
            if (!$this->isCached($templateFile, $this->getCacheId('dbbestseller_manufacturer_'.$id_manufacturer))) {
                $manufacturer = new Manufacturer($id_manufacturer, $id_lang);
                $limit = (int)Configuration::get('DBBESTSELLER_CATEGORY');
                $products = $this->getBestSellerBrand($id_lang, $limit, $id_manufacturer);
                if (is_array($products) && count($products) > 0) {
                    $products = $this->prepareBlocksProducts($products);
                    ...
                    return $this->fetch($templateFile, $this->getCacheId('dbbestseller_manufacturer_'.$id_manufacturer));
                }
            } else {
                return $this->fetch($templateFile, $this->getCacheId('dbbestseller_manufacturer_'.$id_manufacturer));
            }
    ...
    public function hookDisplayBeforeProductList()
    {
        return $this->hookDisplayFooterCategory();
    }

    public function getBestSeller($id_lang, $limit = 20, $id_category = null, $active = true)
    {
    ...
    public function getBestSellerBrand($id_lang, $limit = 20, $id_manufacturer = null, $active = true)
    {
        $sql = 'SELECT p.id_product, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity,
					product_shop.price AS orderprice,
					SUM(od.product_quantity) as sellers
