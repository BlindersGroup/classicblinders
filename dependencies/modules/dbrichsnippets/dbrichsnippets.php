...
class Dbrichsnippets extends Module
{
    ...
    public function hookHeader()
    {
        ...
        if($sectionType == 'index' || ($sectionType == 'category' && empty($category_blog))){
            ...
            if($id_producto > 0) {
                $product = new Product($id_producto, null, $this->context->language->id);
                $valoracion = '';
                ...
                // Rating product
                $sql = "SELECT count(*) as total, SUM(grade) as suma FROM " . _DB_PREFIX_ . "product_comment WHERE id_product = '$id_producto' AND validate = 1";
                ...
                // Valoraciones
                $sql = "SELECT * FROM " . _DB_PREFIX_ . "product_comment WHERE id_product = '$id_producto' AND validate = 1";
                ...
                $json .= '<script type="application/ld+json">{
                    ...
                    "category": "' . $categoria . '",
                    "alternateName": "Compra ' . $product->name . ' a buen precio",
                    "url": "' . $product_url . '",
                    "offers": {    		
                        "@type": "offer",
                        "price": "' . round(Product::getPriceStatic($id_producto), 2) . '",
                        "priceCurrency": "EUR", 
                        "itemOffered": "' . $product->name . '",
